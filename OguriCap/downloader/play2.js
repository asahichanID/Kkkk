...
import { apiYoutubeAudio, apiYoutubeDownload } from '../apiGlobal/index.js';
...
async function downloadAudioBuffer(url) {
  ...
  if (!buffer.length || buffer.length > MAX_ORIGINAL_AUDIO_SIZE) {
    throw new Error('Buffer audio bermasalah atau ukuran melebihi batas');
  }

  return buffer;
}

async function fetchAudioBufferWithFallback(ytUrl) {
  let lastError;

  // 1. Coba apiYoutubeAudio (sumber internal bot - sangat cepat ~1-2 detik)
  try {
    console.log('[PLAY2] [Audio] Mencoba provider utama (apiYoutubeAudio)...');
    const apiRes = await apiYoutubeAudio(ytUrl);
    if (apiRes?.result?.download) {
      console.log('[PLAY2] [Audio] Mengunduh stream audio dari provider utama...');
      const buf = await downloadAudioBuffer(apiRes.result.download);
      console.log(`[PLAY2] [Audio] ✅ Audio berhasil diunduh via apiYoutubeAudio (${(buf.length / (1024 * 1024)).toFixed(2)} MB)`);
      return buf;
    }
  } catch (err) {
    lastError = err;
    console.warn(`[PLAY2] [Audio] ⚠️ apiYoutubeAudio gagal: ${err?.message || err}`);
  }

  // 2. Coba SaveTube (provider cadangan)
  try {
    console.log('[PLAY2] [Audio] Mencoba provider cadangan (SaveTube)...');
    const audio = await savetubeRetry(ytUrl, { downloadType: 'audio', quality: '128kbps' }, 2);
    if (audio?.url) {
      console.log('[PLAY2] [Audio] Mengunduh stream audio dari SaveTube...');
      const buf = await downloadAudioBuffer(audio.url);
      console.log(`[PLAY2] [Audio] ✅ Audio berhasil diunduh via SaveTube (${(buf.length / (1024 * 1024)).toFixed(2)} MB)`);
      return buf;
    }
  } catch (err) {
    lastError = err;
    console.warn(`[PLAY2] [Audio] ⚠️ SaveTube gagal: ${err?.message || err}`);
  }

  // 3. Coba apiYoutubeDownload (provider pamungkas)
  try {
    console.log('[PLAY2] [Audio] Mencoba provider pamungkas (apiYoutubeDownload)...');
    const dlRes = await apiYoutubeDownload(ytUrl, 'mp3');
    if (dlRes?.result?.download) {
      console.log('[PLAY2] [Audio] Mengunduh stream audio dari apiYoutubeDownload...');
      const buf = await downloadAudioBuffer(dlRes.result.download);
      console.log(`[PLAY2] [Audio] ✅ Audio berhasil diunduh via apiYoutubeDownload (${(buf.length / (1024 * 1024)).toFixed(2)} MB)`);
      return buf;
    }
  } catch (err) {
    lastError = err;
    console.warn(`[PLAY2] [Audio] ⚠️ apiYoutubeDownload gagal: ${err?.message || err}`);
  }

  throw new Error(`Semua provider audio gagal. Terakhir: ${lastError?.message || 'Download gagal'}`);
}
...
async function compressAudio(inputBuffer) {
  ...
  console.log(`[PLAY2] [FFmpeg] Memulai kompresi Opus 48k (${(inputBuffer.length / (1024 * 1024)).toFixed(2)} MB)...`);
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.on('close', code => {
      ...
      finished = true;
      console.log(`[PLAY2] [FFmpeg] ✅ Kompresi selesai dalam ${Date.now() - startCompress}ms! Ukuran: ${(output.length / (1024 * 1024)).toFixed(2)} MB`);
      resolve(output);
    });
...
async function createHighQualityThumbnail(conn, thumb) {
  try {
    ...
    if (!conn?.waUploadToServer) return null;
...
async function sendMusicPlayer(conn, m, html) {
  ...
  console.log(`[PLAY2] 📤 Mengirim relayMessage ke ${targetChat} (Response ID: ${responseId})...`);
  ...
  console.log(`[PLAY2] 📨 relayMessage berhasil dikirim ke WhatsApp.`);
}
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) {
    ...
    if (typeof m?.reply === 'function') return m.reply(usage);
    throw usage;
  }

  const startTime = Date.now();
  console.log(`\n================== [PLAY2 STARTED] ==================`);
  console.log(`[PLAY2] 🔎 [1/5] Input pencarian: "${text}"`);
  ...
  try {
    ...
    if (!/youtube\.com|youtu\.be/i.test(text)) {
      console.log(`[PLAY2] 🎵 [2/5] Mencari track di YT Music...`);
      ...
      console.log(`[PLAY2] 🎵 [2/5] Mengambil info video YouTube langsung...`);
    ...
    console.log(`[PLAY2] 🎧 Track: "${title}" - ${artist} (${duration})`);
    console.log(`[PLAY2] 🔗 YouTube: ${ytUrl}`);
    console.log(`[PLAY2] ⚡ [3/5] Mengambil Lirik, Thumbnail, dan Audio secara paralel...`);

    const [syncedLyrics, { imageSrc }, originalBuffer] = await Promise.all([
      // A. Ambil Lirik
      (async () => {
        try {
          console.log('[PLAY2] [Lyrics] Mencari lirik di LRCLIB...');
          let lyrics = [];
          const lrclib = await getLRCLyrics({ title, artist, duration: durationSec, album });

          if (lrclib?.syncedLyrics) {
            lyrics = parseSyncedLyrics(lrclib.syncedLyrics);
          } else if (lrclib?.plainLyrics) {
            lyrics = plainLyricsToSynced(lrclib.plainLyrics, durationSec);
          }

          if (!lyrics.length && trackIdForLyrics) {
            try {
              console.log('[PLAY2] [Lyrics] Mencari lirik cadangan di YTMusic...');
              const ytm = await getYTMusic();
              ...
              if (fallbackLyrics) {
                lyrics = parseSyncedLyrics(fallbackLyrics);
                if (!lyrics.length) lyrics = plainLyricsToSynced(fallbackLyrics, durationSec);
              }
            } catch {}
          }

          lyrics = normalizeLyrics(lyrics);
          console.log(`[PLAY2] [Lyrics] ✅ Lirik selesai (${lyrics.length} baris)`);
          return lyrics;
        } catch (e) {
          console.warn('[PLAY2] [Lyrics] ⚠️ Gagal mengambil lirik:', e.message);
          return [];
        }
      })(),
      ...
      (async () => {
        try {
          console.log('[PLAY2] [Thumb] Mengunduh thumbnail...');
          const thumb = await getThumb(thumbUrl);
          await createHighQualityThumbnail(conn, thumb);
          const iSrc = thumb?.length ? `data:image/jpeg;base64,${thumb.toString('base64')}` : '';
          console.log('[PLAY2] [Thumb] ✅ Thumbnail selesai');
          return { imageSrc: iSrc };
        } catch (e) {
          console.warn('[PLAY2] [Thumb] ⚠️ Gagal memproses thumbnail:', e.message);
          return { imageSrc: '' };
        }
      })(),

      // C. Unduh Audio (Multi-provider fallback)
      fetchAudioBufferWithFallback(ytUrl)
    ]);

    console.log(`[PLAY2] 🗜️ [4/5] Mengompresi audio via FFmpeg...`);
    ...
    console.log(`[PLAY2] 🚀 [5/5] Merender payload HTML & mengirim pesan via relayMessage...`);
    ...
    if (typeof m?.react === 'function') await m.react('✅').catch(() => {});
    console.log(`[PLAY2] ✅ Selesai dalam ${((Date.now() - startTime) / 1000).toFixed(2)} detik! Pesan Music Player terkirim.`);
    console.log(`================== [PLAY2 SUKSES] ==================\n`);
  } catch (error) {
    console.error(`[PLAY2] ❌ Error memproses lagu:`, error?.message || error);
    console.log(`================== [PLAY2 GAGAL] ==================\n`);
    if (typeof m?.react === 'function') await m.react('❌').catch(() => {});
    const errMsg = `❌ Gagal memproses lagu.\n\n> ${error?.message || 'Unknown error'}`;
    if (typeof m?.reply === 'function') {
      await m.reply(errMsg).catch(() => {});
...
export const play2 = async (conn, m, text, prefix, command, db) => {
  return handler(m, {
    conn: conn || global.naze || global.conn,
    text: text || '',
    usedPrefix: prefix || '.',
    command: command || 'play2',
    db
  });
};
